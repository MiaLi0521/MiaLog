...
from mialog.forms import SettingForm, PostForm, CategoryForm, LinkForm
from mialog.extensions import db
from mialog.models import Category, Post, Link, Comment

# 第一个参数为蓝本的名称，第二个参数为包或模块的名称
...
def edit_post(post_id):
    ...
    return render_template('admin/edit_post.html', post_id=post_id, form=form)
...
def manage_comment():
    ...
    if filter_rule == 'unread':
        ...
        filtered_comments = Comment.query.filter_by(form_admin=True)
    else:
        filtered_comments = Comment.query
    pagination = filtered_comments.order_by(Comment.timestamp.desc()).paginate(page=page, per_page=per_page)
    comments = pagination.items
    return render_template('admin/manege_comment.html', comments=comments, pagination=pagination)
